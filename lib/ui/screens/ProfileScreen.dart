    ...
    if(currentTabIndex == 0){
      ...
      GridView.count(
          ...
          children: List.generate(bloc.userPosts.length, (index) {
            ...
            return UIHelper.getProfilePostView(bloc.userPosts[index],context,postAction: (){setState(() {});});
            },
          ))  : Container(
          color: AppColors.backgroundColor,
          width: MediaQuery.of(context).size.width,
      ...
      GridView.count(
          ...
          children: List.generate(bloc.userTextPosts.length, (index) {
            ...
            bloc.userTextPosts[index].postOwnerId = user.userId;
              return UIHelper.getProfilePostView(bloc.userTextPosts[index],context,postAction: (){setState(() {});});
            },
          ))  :
          Container(
              color: AppColors.backgroundColor,    width: MediaQuery.of(context).size.width,
    height: MediaQuery.of(context).size.height * .4,
    child: PlaceHolderWidget(placeHolder: Text(LocalKeys.TEXT_POSTS_PLACE_HOLDER).tr(),));
    }
    else if(currentTabIndex == 2){
      return bloc.userFilePosts.length > 0 ?
